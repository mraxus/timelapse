...
const { Promise: { mapLimit } } = require('./utils')
...
module.exports = {
  ...
  resizeBatch: async function (imageInfoArray, width, processor, outputFolder, progressFunc) {
    ...
    const resizeImage = ({ input, index, width, height }) => {
      ...
      return this.execImageResize(input, outputFile, width, height, processor)
        .then(() => {
          progressFunc(inputFilename, outputFilename, true)
          return { error: null, inputFile: input, outputFile, success: true }
        })
        .catch((error) => {
          progressFunc(inputFilename, outputFilename, false)
          return { error, inputFile: input, outputFile, success: false }
        })
    }

    return mapLimit(inputs, 8, resizeImage)
      .then(results => ({
